(function (global, factory) {
    ...
    constructor(endpoint = defaults.WS_URL, autoConnectMs = RETRY_DELAY, headers = {}, timeout) {
      ...
      Object.defineProperty(this, _onSocketOpen, {
        ...
        value: () => {
          ...
          l$5.debug(() => ['connected to', _classPrivateFieldBase(this, _endpoints)[_endpoints][_classPrivateFieldBase(this, _endpointIndex)[_endpointIndex]]]);
      ...
      _classPrivateFieldBase(this, _eventemitter)[_eventemitter] = new eventemitter3Exports();
  ...
  function retrieveNick(api, accountId) {
    ...
    return (accountId && (_api$query$nicks = api.query.nicks) != null && _api$query$nicks.nameOf ? api.query.nicks.nameOf(accountId) : of(undefined)).pipe(map(nameOf => nameOf != null && nameOf.isSome ? util.u8aToString(nameOf.unwrap()[0]).substring(0, api.consts.nicks.maxLength.toNumber()) : undefined));
  }
  ...
  function callMethod(method, empty) {
    return section => withSection(section, query => () => util.isFunction(query == null ? void 0 : query[method]) ? query[method]() : of(empty));
  }

  const members$5 = callMethod('members', []);

  function prime$4(section) {
    return withSection(section, query => () => util.isFunction(query == null ? void 0 : query.prime) ? query.prime().pipe(map(o => o.unwrapOr(null))) : of(null));
  }
  ...
  function _proposalsFrom(api, query, hashes) {
    return (util.isFunction(query == null ? void 0 : query.proposals) && hashes.length ? combineLatest([of(hashes),
  ...
  function hasProposals$4(section) {
    return withSection(section, query => () => of(util.isFunction(query == null ? void 0 : query.proposals)));
  }
  ...
  function proposal$4(section) {
    return withSection(section, (query, api) => hash => util.isFunction(query == null ? void 0 : query.proposals) ? firstObservable(_proposalsFrom(api, query, [hash])) : of(null));
  }
  ...
  function calcShared(api, bestNumber, data, locks) {
    ...
    return util.objectSpread({}, data, {
      availableBalance: api.registry.createType('Balance', allLocked ? 0 : util.bnMax(new util.BN(0), data != null && data.freeBalance ? data.freeBalance.sub(lockedBalance) : new util.BN(0))),
      lockedBalance,
      lockedBreakdown,
      vestingLocked
    });
  ...
  function account$1(instanceId, api) {
    ...
    return memo(instanceId, address => api.derive.accounts.accountId(address).pipe(switchMap(accountId => {
      ...
      return accountId ? combineLatest([of(accountId), nonDefaultBalances ? queryBalancesAccount(api, accountId, balanceInstances) : util.isFunction((_api$query$system = api.query.system) == null ? void 0 : _api$query$system.account) ? querySystemAccount(api, accountId) : util.isFunction((_api$query$balances = api.query.balances) == null ? void 0 : _api$query$balances.account) ? queryBalancesAccount(api, accountId) : util.isFunction((_api$query$balances2 = api.query.balances) == null ? void 0 : _api$query$balances2.freeBalance) ? queryBalancesFree(api, accountId) : queryNonceOnly(api, accountId)]) : of([api.registry.createType('AccountId'), [api.registry.createType('Index'), [[zeroBalance(api), zeroBalance(api), zeroBalance(api), zeroBalance(api)]]]]);
  ...
  function getImageHashBounded(hash) {
    ...
    : hash.isInline ? hash.asInline.hash.toHex() : util.isString(hash) ? util.isHex(hash) ? hash : util.stringToHex(hash) : util.isU8a(hash) ? util.u8aToHex(hash) : hash.toHex();
  }
  ...
  function dispatchQueue(instanceId, api) {
    return memo(instanceId, () => {
      ...
      return util.isFunction((_api$query$scheduler = api.query.scheduler) == null ? void 0 : _api$query$scheduler.agenda) ? queryScheduler(api) : api.query.democracy.dispatchQueue ? queryQueue(api) : of([]);
    });
  }
  ...
  function preimages(instanceId, api) {
    return memo(instanceId, hashes => hashes.length ? util.isFunction(api.query.democracy.preimages) ? getDemocracyImages(api, hashes) : util.isFunction(api.query.preimage.preimageFor) ? getImages(api, hashes) : of([]) : of([]));
  }
  ...
  function referendumIds(instanceId, api) {
    return memo(instanceId, () => {
      var _api$query$democracy;
      return (_api$query$democracy = api.query.democracy) != null && _api$query$democracy.lowestUnbaked ? api.queryMulti([api.query.democracy.lowestUnbaked, api.query.democracy.referendumCount]).pipe(map(([first, total]) => total.gt(first)
  ...
  function overview$1(instanceId, api) {
    return memo(instanceId, () => {
      ...
      return (_api$query$registrar = api.query.registrar) != null && _api$query$registrar.parachains && api.query.parachains ? api.query.registrar.parachains().pipe(switchMap(paraIds => combineLatest([of(paraIds), api.query.parachains.didUpdate(), api.query.registrar.paras.multi(paraIds), api.query.registrar.pendingSwap.multi(paraIds), api.query.parachains.relayDispatchQueueSize.multi(paraIds)])), map(parse$1)) : of([]);
    });
  }
  ...
  function parseRewards(api, stashId, [erasPoints, erasPrefs, erasRewards], exposures) {
    return exposures.map(({
      ...
      Object.entries(eraValidators).forEach(([validatorId, exposure]) => {
        ...
        const valComm = ((_allValPrefs$validato = allValPrefs[validatorId]) == null ? void 0 : _allValPrefs$validato.commission.unwrap()) || util.BN_ZERO;
        const expTotal = ((_exposure$total = exposure.total) == null ? void 0 : _exposure$total.unwrap()) || util.BN_ZERO;
  ...
  function createClass({
    ...
    class Submittable extends ExtrinsicBase {
      ...
      get hasDryRun() {
        var _api$rpc$system;
        return util.isFunction((_api$rpc$system = api.rpc.system) == null ? void 0 : _api$rpc$system.dryRun);
      }
      get hasPaymentInfo() {
        var _api$call$transaction;
        return util.isFunction((_api$call$transaction = api.call.transactionPaymentApi) == null ? void 0 : _api$call$transaction.queryInfo);
      }
  ...
  class Decorate extends Events {
    ...
    constructor(options, type, decorateMethod) {
      ...
      this.#registry = ((_options$source = options.source) == null ? void 0 : _options$source.registry) || options.registry || new types.TypeRegistry();
